...
def create_cache():
    set_env_vars()
    install_dir = Path('./cashing').resolve()
    ...
    spack_config = SpackConfig(env=SpackDescriptor('twin-spack-env', install_dir, spack_env_git),
                               repos=[SpackDescriptor('ebrains-spack-builds', install_dir, ebrains_spack_builds_git)],
                               install_dir=install_dir,
                               upstream_instance=None,
                               system_name='VBT',
                               concretization_dir=install_dir / 'concretize_cache',
                               buildcache_dir=install_dir / 'binary_cache',
